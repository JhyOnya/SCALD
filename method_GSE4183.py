...
class p_A_x_func(nn.Module):
    ...
    def forward(self, x, drop=True):
        ...
        pre = torch.mul(x.unsqueeze(1),
                        (A / torch.mean(A, dim=0, keepdim=True)).T.unsqueeze(0))  # [n, d, md] = [n, 1, d] x [1, d, md]
...
def train(data, args, device, **kw):
    ...
    while True:
        ...
        data_batch = torch.tensor(data_np, dtype=torch.float32, requires_grad=False, device=device)
        ...
        l_pre_epoch = loss_list['l'][-1]
        l_A_pre_epoch = loss_list['l_A'][-1]
        l_dag_adv_pre_epoch = loss_list['l_dag_adv'][-1]
        l_dag_com_pre_epoch = loss_list['l_dag_com'][-1]
