...
class LocallyConnected(nn.Module):
    ...
    def forward(self, input):
        ...
        self.weight = Parameter(self.weight.clamp(1., 2.))
...
class p_A_x_func(nn.Module):
    ...
    def forward(self, x, drop=True):
        ...
        x_ = torch.mul(x.unsqueeze(1), A.T.unsqueeze(0))  # [n, d, md] = [n, 1, d] x [1, d, md]
        pre = self.inp(x_)  # [n, d, ml] = [n, d, md] x [md, ml]
...
def train(data, args, device, max_comp=50, **kw):
    def h_dag_com(A, orient_adv_gpu, orient_dec_gpu, **kw):
        B = torch.matrix_power(orient_adv_gpu * A / d + torch.eye(d, device=device, requires_grad=False), d)

        C_ = (orient_dec_gpu * A / d) @ B
        C = torch.matrix_power(C_ @ C_ + torch.eye(d, device=device, requires_grad=False), int(d / 2))

        # print(torch.trace(B).sum() - d, torch.trace(C).sum() - d)
    ...
    optimizer = optim.Adam(params, lr=2e-3, weight_decay=1e-4)  # lr 学习率 wd 权重衰减
    ...
    while True:
        ...
        print("epoch: %4s  l=%-10f l_A=%-10f l_dag_adv=%-10f l_dag_com=%-10f "
